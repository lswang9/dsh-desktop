...
describe('Feishu release notes pipeline', () => {
  ...
  it(
    ...
    () => {
      ...
      expect(output).toContain("You are Pierhouse's Release Bot.")
      expect(output).toContain('## Pierhouse v0.4.0 Release Note')
      expect(output).toContain('📢 大家可以直接在客户端中更新。')
      expect(output).toContain('📢 You can update directly from the Pierhouse app.')
  ...
  it('generates deterministic fallback release notes that pass validation', () => {
    ...
    try {
      ...
      expect(content).toContain('## Pierhouse v0.4.0 Release Note')
      expect(content).toContain('📢 大家可以直接在客户端中更新。')
      expect(content).toContain('📢 You can update directly from the Pierhouse app.')
  ...
  it('rejects invalid markdown with links or incorrect headings', () => {
    ...
    try {
      ...
      const invalidContent = `## Pierhouse v0.4.0 Release Note
...
## Pierhouse v0.4.0 Release Note

📢 You can update directly from the Pierhouse app.
...
`
  ...
  it('generates deterministic fallback prerelease notes that pass validation', () => {
    ...
    try {
      ...
      const content = readFileSync(tempFile, 'utf8')
      expect(content).toContain('## Pierhouse v0.7.2（预发布）Release Note')
  ...
  it(
    ...
    () => {
      ...
      expect(output).toContain('## Pierhouse v0.7.2（预发布）Release Note')
